...
def clean_answer(model_pred, random_guess=False):
    ...
    if "Thus, yes." in model_pred:
        ...
        print("Warning: answer trigger not found in model prediction:", model_pred,
              "; returning yes/no based on exact match of `no`.", flush=True)
        if random_guess:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--amateur_model_name", type=str, default="huggyllama/llama-7b")
    ...
    llm = ContrastiveDecoding(model_name, amateur_model_name, device, num_gpus, args.max_gpu_memory)
    stop_word_list = ["Q:", "\n\n##"]
